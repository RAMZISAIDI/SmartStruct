    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
      <div>
        ...
        <div class="card" style="margin-bottom:1rem">
          ...
          <div class="form-grid-2">
            <div class="form-group">
              <label class="form-label">${L('اسم البنك','Nom de la banque')}</label>
              <input class="form-input" id="setBankName" value="${escHtml(tenant?.bank_name||'')}" placeholder="${L('CCP, BNA, BEA...','CCP, BNA, BEA...')}">
            </div>
            <div class="form-group">
              <label class="form-label">${L('رقم الحساب البنكي (RIB)','Numéro de compte (RIB)')}</label>
              <input class="form-input" id="setBankAccount" value="${escHtml(tenant?.bank_account||'')}" placeholder="000 00000 0000000000 00" dir="ltr">
            </div>
          </div>
        ...
        <!-- Branding: Logo + Stamp -->
        <div class="card" style="margin-bottom:1rem">
          <div style="font-weight:800;margin-bottom:.3rem">🎨 ${L('شعار وختم الشركة','Logo et tampon')}</div>
          <div style="font-size:.78rem;color:var(--dim);margin-bottom:1rem">
            ${L('سيظهران تلقائياً في الفواتير وقسائم الرواتب وكل المستندات المطبوعة. الحجم الأقصى 500KB. الأنواع المدعومة: PNG, JPG, SVG, WEBP.','Apparaîtront automatiquement sur les factures, fiches de paie et tous les documents imprimés.')}
          </div>

          <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
            <!-- Logo -->
            <div>
              <div style="font-weight:700;margin-bottom:.5rem;font-size:.9rem">📷 ${L('شعار الشركة','Logo')}</div>
              <div id="logoPreview" style="width:100%;height:140px;background:rgba(255,255,255,.04);border:2px dashed var(--border2);border-radius:10px;display:flex;align-items:center;justify-content:center;overflow:hidden;margin-bottom:.5rem">
                ${tenant?.logo_url
                  ? `<img src="${tenant.logo_url}" style="max-width:100%;max-height:100%;object-fit:contain">`
                  : `<div style="text-align:center;color:var(--dim)"><div style="font-size:2.5rem;margin-bottom:.3rem">🏢</div><div style="font-size:.78rem">${L('لا يوجد شعار','Aucun logo')}</div></div>`}
              </div>
              <input type="file" id="logoFile" accept="image/png,image/jpeg,image/jpg,image/svg+xml,image/webp" style="display:none" onchange="handleLogoUpload(this)">
              <div style="display:flex;gap:.4rem">
                <button class="btn btn-gold btn-sm" style="flex:1" onclick="document.getElementById('logoFile').click()">📤 ${L('رفع','Choisir')}</button>
                ${tenant?.logo_url ? `<button class="btn btn-ghost btn-sm" onclick="if(confirm('${L('حذف الشعار؟','Supprimer le logo ?')}')){CompanyBranding.removeLogo();setTimeout(()=>App.render(),500)}">🗑️</button>` : ''}
              </div>
            </div>

            <!-- Stamp -->
            <div>
              <div style="font-weight:700;margin-bottom:.5rem;font-size:.9rem">🖋️ ${L('الختم','Tampon')}</div>
              <div id="stampPreview" style="width:100%;height:140px;background:rgba(255,255,255,.04);border:2px dashed var(--border2);border-radius:10px;display:flex;align-items:center;justify-content:center;overflow:hidden;margin-bottom:.5rem">
                ${tenant?.stamp_url
                  ? `<img src="${tenant.stamp_url}" style="max-width:100%;max-height:100%;object-fit:contain">`
                  : `<div style="text-align:center;color:var(--dim)"><div style="font-size:2.5rem;margin-bottom:.3rem">🖋️</div><div style="font-size:.78rem">${L('لا يوجد ختم','Aucun tampon')}</div></div>`}
              </div>
              <input type="file" id="stampFile" accept="image/png,image/jpeg,image/jpg,image/svg+xml,image/webp" style="display:none" onchange="handleStampUpload(this)">
              <div style="display:flex;gap:.4rem">
                <button class="btn btn-gold btn-sm" style="flex:1" onclick="document.getElementById('stampFile').click()">📤 ${L('رفع','Choisir')}</button>
                ${tenant?.stamp_url ? `<button class="btn btn-ghost btn-sm" onclick="if(confirm('${L('حذف الختم؟','Supprimer le tampon ?')}')){const t=Auth.getTenant();const ts=DB.get('tenants').map(x=>x.id===t.id?{...x,stamp_url:null}:x);DB.setSilent('tenants',ts);Toast.success('🗑️ تم الحذف');setTimeout(()=>App.render(),500)}">🗑️</button>` : ''}
              </div>
            </div>
          </div>
        </div>
...
function saveLegalSettings() {
  ...
  const bank_name    = document.getElementById('setBankName')?.value || '';
  const bank_account = document.getElementById('setBankAccount')?.value || '';

  const tenants = DB.get('tenants').map(t=>t.id===tid?{...t, nif, nis, tva_rate: parseFloat(tva), address, bank_name, bank_account }:t);
...
async function handleLogoUpload(input) {
  ...
  if (typeof CompanyBranding === 'undefined') {
    Toast.error('⚠️ ملف features.js غير محمّل');
    return;
  }
  const ok = await CompanyBranding.saveLogo(file);
  input.value = '';
  if (ok) setTimeout(() => App.render(), 600);
}
...
async function handleStampUpload(input) {
  ...
  const ok = await CompanyBranding.saveStamp(file);
  input.value = '';
  if (ok) setTimeout(() => App.render(), 600);
}
...
Pages.salary = function() {
  ...
  // ✅ استخدام AlgerianPayroll الجديد بدلاً من الحساب البسيط
  const calcSalary = w => {
    if (typeof AlgerianPayroll !== 'undefined') {
      return AlgerianPayroll.calculate(w, monthAtt);
    }
    ...
    if (w.contract_type==='monthly') return {presentDays:pDays, halfDays:hDays, baseSalary:w.monthly_base, grossSalary:w.monthly_base, netSalary:w.monthly_base, total:w.monthly_base, cnasEmployee:0, irg:0};
    const total = Math.round((pDays+hDays*0.5)*Number(w.daily_salary));
    return {presentDays:pDays, halfDays:hDays, baseSalary:total, grossSalary:total, netSalary:total, total, cnasEmployee:0, irg:0};
  };

  const total = workers.reduce((s,w)=>s+(calcSalary(w).netSalary || calcSalary(w).total || 0), 0);
  const totalGross = workers.reduce((s,w)=>s+(calcSalary(w).grossSalary || 0), 0);
  const totalCNAS = workers.reduce((s,w)=>s+(calcSalary(w).cnasEmployee || 0), 0);
  const totalIRG  = workers.reduce((s,w)=>s+(calcSalary(w).irg || 0), 0);
  ...
  return layoutHTML('salary', 'الرواتب', `
    <div class="page-header">
      <div><div class="page-title">💵 الرواتب الشهرية</div><div class="page-sub">${monthLabel} • حساب جزائري قانوني (CNAS + IRG)</div></div>
      <div class="page-actions">
    ...
    <div class="stats-grid" style="grid-template-columns:repeat(4,1fr)">
      <div class="stat-card"><div class="stat-icon">💵</div><div class="stat-value" style="color:var(--green);font-size:1rem">${fmt(total)}</div><div class="stat-label">صافي الرواتب (دج)</div></div>
      <div class="stat-card"><div class="stat-icon">📊</div><div class="stat-value" style="color:var(--blue);font-size:1rem">${fmt(totalGross)}</div><div class="stat-label">إجمالي قبل الخصم</div></div>
      <div class="stat-card"><div class="stat-icon">🏥</div><div class="stat-value" style="color:#FF7043;font-size:1rem">${fmt(totalCNAS)}</div><div class="stat-label">CNAS (9%)</div></div>
      <div class="stat-card"><div class="stat-icon">📋</div><div class="stat-value" style="color:#9B6DFF;font-size:1rem">${fmt(totalIRG)}</div><div class="stat-label">IRG</div></div>
    </div>
    <div style="display:grid;gap:.8rem">
      ${workers.map(w => {
        const calc = calcSalary(w);
        const net = calc.netSalary || calc.total || 0;
        ...
        return `<div class="salary-card">
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem">
            ...
            <div style="display:flex;gap:.5rem;align-items:center;flex-wrap:wrap">
              <button class="btn btn-ghost btn-sm" onclick="Payslip.generate(${w.id},'${selectedMonthKey}')" title="طباعة قسيمة راتب احترافية">
                🖨️ قسيمة
              </button>
              <div style="text-align:left">
                <div style="font-size:1.1rem;font-weight:900;color:var(--green);font-family:monospace">${fmt(net)} دج</div>
                ${paid?`<span class="badge badge-active">✅ تم الصرف</span>`:canDo('salary')?`<button class="btn btn-gold btn-sm" onclick="paySalary(${w.id},'${selectedMonthKey}',${net})">💳 صرف</button>`:'<span class="badge">معلق</span>'}
              </div>
            </div>
          </div>
          <div class="salary-breakdown">
            <div class="salary-line"><span>أيام الحضور</span><span>${calc.presentDays || 0} يوم${calc.halfDays?` + ${calc.halfDays} نصف يوم`:''}${calc.overtimeHours?` • ${calc.overtimeHours} س.إضافية`:''}</span></div>
            <div class="salary-line"><span>الأجر الإجمالي (Brut)</span><span>${fmt(calc.grossSalary || 0)} دج</span></div>
            ${calc.cnasEmployee ? `<div class="salary-line" style="color:#FF7043"><span>− CNAS (9%)</span><span>${fmt(calc.cnasEmployee)} دج</span></div>` : ''}
            ${calc.irg ? `<div class="salary-line" style="color:#9B6DFF"><span>− IRG (الضريبة)</span><span>${fmt(calc.irg)} دج</span></div>` : ''}
            <div class="salary-line total-line"><span>الصافي المستحق</span><span>${fmt(net)} دج</span></div>
          </div>
        </div>`;
      }).join('')}
    </div>
...
function printInvoiceWindow(id) {
  // ✅ استخدم النظام الجديد ProfessionalInvoice إذا كان متوفراً
  if (typeof ProfessionalInvoice !== "undefined" && ProfessionalInvoice.generate) {
    return ProfessionalInvoice.generate(id);
  }
  // fallback إذا لم تُحمّل features.js لسبب ما
  Toast.error("⚠️ خطأ: ملف الميزات لم يُحمّل. الرجاء تحديث الصفحة.");
}
