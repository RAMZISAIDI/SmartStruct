...
//  يحفظ كل وثيقة في جدول documents مع البيانات الأصلية لإعادة الطباعة
// ════════════════════════════════════════════════════════════════════
...
window.DZArchive = {
  ...
  save(kind, opts) {
    if (typeof Auth === 'undefined' || !Auth.getUser()) {
      console.warn('[DZArchive] Auth not available');
      return null;
    }
    const reg = DZ_DOC_REGISTRY[kind];
    if (!reg) {
      console.warn('[DZArchive] Unknown kind:', kind);
      return null;
    }
    ...
    // تخزين البيانات الكاملة لإعادة الطباعة
    ...
    try {
      ...
      if (typeof sbSync === 'function') {
        sbSync('documents', record, 'POST').catch(e => console.warn('[DZArchive] sync failed:', e.message));
      }

      if (typeof AuditLog !== 'undefined' && AuditLog.log) {
        AuditLog.log('archive', 'documents', record.id, null, { kind, number, category: reg.category });
      }
  ...
  reprint(id) {
    ...
    if (!doc) {
      if (typeof Toast !== 'undefined') Toast.error('الوثيقة غير موجودة');
      return false;
    }
    const reg = DZ_DOC_REGISTRY[doc.doc_kind];
    if (!reg) {
      if (typeof Toast !== 'undefined') Toast.error('نوع وثيقة غير معروف');
      return false;
    }
  ...
  delete(id) {
    ...
    if (typeof sbSyncDelete === 'function') {
      sbSyncDelete('documents', id).catch(e => console.warn('[DZArchive] delete sync failed:', e.message));
    }
    if (typeof AuditLog !== 'undefined' && AuditLog.log) {
      ...
      if (!opts._isReprint) {
        try {
          ...
          if (kinds.length > 1 && opts.kind) {
            const matchKind = {
              'pro': 'pv_reception_pro', 'final': 'pv_reception_def',
              'cdd': 'cdd', 'cta': 'cta', 'cdi': 'cdi',
            }[opts.kind];
            if (matchKind && kinds.includes(matchKind)) kind = matchKind;
          }
